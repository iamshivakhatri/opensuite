...
Document-scoped threads are constrained by composite FK `(document_id, workspace_id) → document(id, workspace_id)` (when `document_id` is set) so a thread cannot attach a document from another workspace. Persistence access for agents lives in `apps/api` (`createAgentPersistenceService`); durable execution is `createAgentExecutionService`; HTTP surface is `registerAgentRoutes` (sync runs, no SSE yet).

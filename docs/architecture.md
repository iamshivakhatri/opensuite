...
Authentication uses Better Auth's official React client (`better-auth/react` → `createAuthClient`), not a hand-rolled auth layer. `src/lib/auth-client.ts` configures `baseURL: NEXT_PUBLIC_API_URL` (apps/api's origin); `useSession()` drives client-side session restoration and route gating (`app/(auth)/layout.tsx` vs `app/app/layout.tsx`), and `signIn.email` / `signUp.email` / `signOut` drive the forms. The apps are separate origins locally and in production, but production `www.opensuite.tech` and `api.opensuite.tech` are the same site, so Better Auth uses its default Secure, HttpOnly, SameSite=Lax cookie settings (`AUTH_CROSS_ORIGIN=false`). API CORS permits only `WEB_ORIGIN`, and Better Auth trusts that same origin. `apps/web`'s own `GET /api/me` calls (`src/lib/api.ts`) must set `credentials: "include"` explicitly since they're plain `fetch`, not Better Auth client calls.
...
Document-scoped threads are constrained by composite FK `(document_id, workspace_id) → document(id, workspace_id)` (when `document_id` is set) so a thread cannot attach a document from another workspace. Persistence access for agents lives in `apps/api` (`createAgentPersistenceService`); durable execution is `createAgentExecutionService`; live tracking/SSE is `createAgentRunManager` (in-process only — restarts abandon active memory).
